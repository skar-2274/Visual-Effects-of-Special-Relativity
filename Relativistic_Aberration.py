...
# Step 1: Define the function f(phi, B)
def f(phi, B):
    return np.arccos((np.cos(phi) + B) / (1 + B * np.cos(phi)))

# Step 2: Transform input from polar coordinates (r, phi) to Cartesian (x, y)
def to_polar(x, y):
    r = np.sqrt(x**2 + y**2)
    phi = np.arctan2(y, x)
    return r, phi
...
# Step 4: Create a figure and axes for both the plot and the image
